...
export const NumberSelector: React.FC<NumberSelectorProps> = ({
  ...
  const digitPresets = [
    { label: isBn ? 'ডাবল: 7-7-2 (রিপিট)' : 'Double: 7-7-2', numbers: [7, 7, 2] },
    { label: isBn ? 'ডাবল: 0-5-0 (রিপিট)' : 'Double: 0-5-0', numbers: [0, 5, 0] },
    { label: isBn ? 'ট্রিপল: 3-3-3 (সব রিপিট)' : 'Triple: 3-3-3', numbers: [3, 3, 3] },
    { label: isBn ? 'সিঙ্গেল: 3-7-2' : 'Single: 3-7-2', numbers: [3, 7, 2] },
  ];

  const lottoPresets = [
    { label: isBn ? 'ক্লাসিক ড্র' : 'Classic Draw', numbers: [3, 7, 12, 16, 21, 25].filter((n) => n <= poolSize).slice(0, pickSize) },
