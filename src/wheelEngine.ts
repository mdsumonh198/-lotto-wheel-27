...
export const GAME_PRESETS: GamePreset[] = [
  ...
  },
  {
    ...
    allowRepeats: false,
    orderMatters: false,
    smartStops: [14, 135, 500, 2331],
    description: 'Guaranteed 5-Match Full Lock with 2,331 tickets (100% Schönheim Lower Bound). Top choice for syndicates.',
  },
  {
    id: '6-20',
...
export const TARGET_WHEEL_SIZE = 2331;
...
export function calculateGoalRequirements(
  ...
  if (poolSize === 27 && pickSize === 6) {
    const milestones: GoalMilestone[] = [
      ...
      { tier: 5, frequency: 1, tickets: 2331, label: '5-Match (1x)', description: 'Absolute Full Lock: 100% guaranteed ≥ 1 ticket with 5-match' },
    ];
    ...
    if (matchTier === 3) {
      ...
      recommended = targetFrequency === 1 ? 2331 : Math.round(2331 * targetFrequency);
    } else {
...
export function evaluateWheel(
  ...
  return { evaluations, fullMatchCounts, budgetMatchCounts, goalSummary };
}
